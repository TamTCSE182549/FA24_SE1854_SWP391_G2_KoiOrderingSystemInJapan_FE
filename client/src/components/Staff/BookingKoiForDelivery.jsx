    ...
    const fetchBookings = async () => {
      try {
        const response = await axios.get(
          `http://localhost:8080/bookings/koi/list/delivery`,
          {
            headers: {
    ...
    const fetchDepositsInfo = async () => {
      try {
        ...
        for (const booking of bookings) {
          try {
            const response = await axios.get(
              `http://localhost:8080/deposit/${booking.id}`,
              {
                headers: {
                  Authorization: `Bearer ${token}`,
                },
            ...
            console.log(
              `Deposit data for booking ${booking.id}:`,
              response.data
            );
  ...
  const handleDelete = async (bookingId) => {
    Modal.confirm({
      ...
      async onOk() {
        try {
          await axios.put(
            `http://localhost:8080/bookings/delete/koi/${bookingId}`,
            null,
            {
              headers: {
                Authorization: `Bearer ${token}`,
